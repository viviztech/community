...
class Event(models.Model):
    ...
    def available_tickets(self):
        booked = self.registrations.filter(
            payment_status='completed'
        ).aggregate(total=models.Sum('number_of_delegates'))['total'] or 0
...
class EventRegistration(models.Model):
    ...
    payment = models.OneToOneField(
        ...
        null=True, blank=True, related_name='event_registration'
    )
    ...
    def generate_ticket(self):
        ...
        import uuid
        self.ticket_number = f"TKT/{self.event.id.hex[:8].upper()}/{uuid.uuid4().hex[:6].upper()}"
