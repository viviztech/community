...
class MembershipTier(models.Model):
    ...
    tier_type = models.CharField(
        max_length=20, choices=TierType.choices, unique=True
    )
    description = models.TextField()
    yearly_price = models.DecimalField(max_digits=10, decimal_places=2)
    lifetime_price = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)
...
class Membership(models.Model):
    ...
    tier = models.ForeignKey(
        MembershipTier, on_delete=models.PROTECT,
        related_name='memberships'
    )
    ...
    payment = models.OneToOneField(
        ...
        null=True, blank=True, related_name='membership'
    )
    ...
    def generate_certificate(self):
        ...
        import uuid
        self.certificate_number = f"ACTIV/{timezone.now().year}/{uuid.uuid4().hex[:6].upper()}"
