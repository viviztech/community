...
class ApprovalWorkflow(models.Model):
    ...
    member = models.ForeignKey(
        "members.Member", on_delete=models.CASCADE, related_name="approval_workflows"
    )
    current_level = models.CharField(
        max_length=20, choices=Level.choices, default=Level.BLOCK
    )
    ...
    def __str__(self):
        return (
            f"Workflow for {self.member.user.full_name} - {self.get_status_display()}"
        )
...
class ApprovalAction(models.Model):
    ...
    workflow = models.ForeignKey(
        ApprovalWorkflow, on_delete=models.CASCADE, related_name="actions"
    )
    ...
    level = models.CharField(max_length=20, choices=ApprovalWorkflow.Level.choices)
