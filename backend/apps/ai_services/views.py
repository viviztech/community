...
class AnalyticsView(APIView):
    ...
    def _get_geographic_distribution(self):
        ...
        districts = (
            Member.objects.filter(user__is_active=True, district__isnull=False)
            .values("district__name")
            .annotate(count=Count("id"))
        )
        return [
            {"district": d["district__name"], "count": d["count"]} for d in districts
        ]
    ...
    def _get_predictions(self):
        ...
        growth_rate = (
            ((current_month - last_month) / max(last_month, 1)) * 100
            if last_month > 0
            else 0
        )
...
class ChatbotView(APIView):
    ...
    def _generate_response(self, query):
        ...
        if any(
            word in query for word in ["document", "document", "aadhar", "pan", "gst"]
        ):
            return (
...
class BusinessMatchingView(APIView):
    ...
    def _find_business_matches(self, member, limit=10):
        ...
        members = (
            Member.objects.filter(user__is_active=True, is_doing_business=True)
            .exclude(id=member.id)
        ...
        for m in members:
            ...
            if member.business_activities and m.business_activities:
                if any(
                    word in m.business_activities.lower()
                    for word in member.business_activities.lower().split()[:5]
                ):
                    score += 0.2
