...
class TestChatbotView:
    ...
    def test_chatbot_responds_to_membership_query(self):
        ...
        request = self.factory.post('/api/v1/ai/chatbot/', {
            'query': 'How do I register for membership?'
        }, format='json')
        response = self.view(request)
        assert response.status_code == 200
        assert 'membership' in response.data['response'].lower() or 'register' in response.data['response'].lower()
    ...
    def test_chatbot_responds_to_payment_query(self):
        ...
        response = self.view(request)
        assert response.status_code == 200
        assert 'fee' in response.data['response'].lower() or 'cost' in response.data['response'].lower() or 'price' in response.data['response'].lower()
    
    def test_chatbot_default_response(self):
        """Test chatbot default response for unknown queries."""
        request = self.factory.post('/api/v1/ai/chatbot/', {
            'query': 'xyzabc123 random query'
        }, format='json')
        response = self.view(request)
        assert response.status_code == 200
...
class TestAIIntegration:
    ...
    def test_chatbot_conversation_flow(self):
        ...
        for query in queries:
            request = factory.post('/api/v1/ai/chatbot/', {'query': query}, format='json')
