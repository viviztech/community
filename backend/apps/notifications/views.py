...
class NotificationService:
    ...
    def send_notification(user, event_type, context, channels=None):
        ...
        for channel in channels:
            if channel == 'email' and template.channel in ['email', NotificationTemplate.Channel.EMAIL]:
            ...
            elif channel == 'sms' and template.channel in ['sms', NotificationTemplate.Channel.SMS]:
                body = NotificationService.render_template(template.body, context)
                results['sms'] = NotificationService.send_sms(user, body)
            
            elif channel == 'whatsapp' and template.channel in ['whatsapp', NotificationTemplate.Channel.WHATSAPP]:
                body = NotificationService.render_template(template.body, context)
