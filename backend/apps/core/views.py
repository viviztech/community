...
class AdminDashboardStatsView(APIView):
    ...
    def get(self, request):
        ...
        has_admin_area = (
            ...
            (user.member_profile.block or user.member_profile.district or user.member_profile.state)
        )
        ...
        if not is_superadmin:
            if user.member_profile.block:
                member_queryset = member_queryset.filter(block=user.member_profile.block)
            elif user.member_profile.district:
                member_queryset = member_queryset.filter(district=user.member_profile.district)
            elif user.member_profile.state:
                member_queryset = member_queryset.filter(state=user.member_profile.state)
        
        # Total members
        stats['total_members'] = member_queryset.count()
        
        # Approved members
        stats['approved_members'] = member_queryset.filter(
            user__is_active=True
        ).count()
        
        # Pending approvals
        pending_approvals = ApprovalWorkflow.objects.filter(status='pending', is_active=True)
        if not is_superadmin:
            if user.member_profile.block:
                pending_approvals = pending_approvals.filter(
                    member__block=user.member_profile.block,
                    current_level='block'
                )
            elif user.member_profile.district:
                pending_approvals = pending_approvals.filter(
        ...
        tier_stats = Membership.objects.filter(
            status='active'
        ).values('tier__name').annotate(
            count=Count('id')
        )
        stats['membership_tiers'] = {item['tier__name']: item['count'] for item in tier_stats}
        ...
        stats['monthly_revenue'] = monthly_payments.aggregate(
            total=Sum('amount')
        )['total'] or 0
        
        # Total revenue
        stats['total_revenue'] = Payment.objects.filter(
            status='completed'
...
class AdminGeographicStatsView(APIView):
    ...
    def get(self, request):
        ...
        # Members by district
        by_district = queryset.values('district__name').annotate(
            count=Count('id')
        ).exclude(district__name__isnull=True)
        
        # Members by block
        by_block = queryset.values('block__name').annotate(
            count=Count('id')
        ).exclude(block__name__isnull=True)
        
        # Members by social category
        by_category = queryset.values('social_category').annotate(
            count=Count('id')
